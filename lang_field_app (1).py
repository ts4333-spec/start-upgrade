...
if "api_item" in st.session_state:
    ...
    if run_btn:
        ...
        final_item = {
            ...
            "itemId":       item.get("itemId") or "",   # HTML 크롤링 AuthorId 파싱용
            "isbn13":       st.session_state.get("isbn") or "",  # itemId 없을 때 fallback
            # API가 반환한 subInfo 원본 유지 (authors 배열 포함)
            # originalTitle이 수정됐으면 subInfo에 반영
            "subInfo": {
                **(item.get("subInfo") or {}),
