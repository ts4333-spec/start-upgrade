...
def aladin_lookup(isbn13: str) -> dict | None:
    """알라딘 TTB API로 도서 정보 조회. 실패 시 화면에 에러 원인 출력."""
    if not ALADIN_TTB_KEY:
        return None
    try:
        params = {
            "ttbkey":    ALADIN_TTB_KEY.strip(),
            "itemIdType": "ISBN13",
            "ItemId":    isbn13,
            "output":    "js",
            "Version":   "20131101",
            "OptResult": "authors",  # 기존 "subInfo"는 잘못된 파라미터라 "authors"로 수정
        }
        r = requests.get(
        ...
        # 알라딘 서버에서 API 에러 메시지를 뱉은 경우 화면에 출력
        if "errorCode" in data:
            st.error(f"⚠️ 알라딘 API 거부: {data.get('errorMessage')} (에러코드: {data.get('errorCode')})")
            return None
        ...
        st.error(f"⚠️ API 통신 중 예기치 않은 오류 발생: {e}")
